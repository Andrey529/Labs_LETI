...
int main()
{
    const double startPoint = 3*M_PI/2, endPoint = 15*M_PI, step = M_PI/8;
    const unsigned size = (unsigned)((endPoint-startPoint)/step) + 1;
    double *argument, *result, maximum;
    int i, gD, gM;
    unsigned scaleX, scaleY, skipX, skipY, clipoff = 40;
    ...
    result[0] = pow(sin(argument[0]/2),2)+sqrt(argument[0]);

    for (i = 1; i<size; i++)
    {
        argument[i] = argument[i-1]+step;
        result[i] = pow(sin(argument[i]/2),2)+sqrt(argument[i]);
    }
