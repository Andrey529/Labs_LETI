...
def cinterp(phi, dphi, a0, a1):
    ...
    if np.sqrt(d1 ** 2 - dphi(a0) * dphi(a1)) < 0:
        a = a0
        return a
...
def prsearch(f, df, x0, tol):
    ...
    while True:
        a_next = wolfesearch(f, df, x, p, 3, c1, c2)
        x_next = x + a_next * p
        g_next = df(x_next)

        betta = (np.dot(np.transpose(g_next - g), g_next)[0][0]) / (np.dot(np.transpose(g), g)[0][0])
        p_next = -1 * g_next + betta * p

        coordinates.append(x_next)

        if not (norm(df(x)) >= tol and neval < kmax):
            break
        else:
            neval = neval + 1
            x = x_next
    ...
    fmin = f(x_next)
    answer_ = [x_next, fmin, neval, coordinates]
    return answer_
